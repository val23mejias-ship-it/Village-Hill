// src/audio.js — Sonidos ambientales con Web Audio API (sin archivos externos)
let ctx = null;
let masterGain = null;
...
export function startAmbience() {
  ...
  // ── Viento bajo (ruido filtrado) ──────────────────────────────
  playWind();
 
  // ── Tonos graves inquietantes ─────────────────────────────────
  playDrone(55,  0.06);
  playDrone(82,  0.04);
  playDrone(110, 0.03);
  ...
  // ── Latidos del corazón (suaves, lejanos) ─────────────────────
  scheduleHeartbeat();
}
...
function scheduleCreaks() {
  function creak() {
    ...
    // Próximo crujido en 4–15 segundos
    setTimeout(creak, 4000 + Math.random() * 11000);
  }
  setTimeout(creak, 2000 + Math.random() * 4000);
}
